...
class HttpRuntimeProxyTest(wsgi_test_utils.WSGITestCase):
  ...
  def _test_start_and_quit(self, quit_with_sigterm):
    ...
    self.mox.ReplayAll()
    self.proxy.start()
    self.assertEqual(30000, self.proxy._proxy._port)
    self.assertEqual('localhost', self.proxy._proxy._host)
    self.mox.VerifyAll()
    self.mox.ResetAll()
  ...
  def test_start_with_host_port(self):
    ...
    self.mox.ReplayAll()
    self.proxy.start()
    self.assertEqual(34567, self.proxy._proxy._port)
    self.assertEqual('::1', self.proxy._proxy._host)
    self.mox.VerifyAll()
