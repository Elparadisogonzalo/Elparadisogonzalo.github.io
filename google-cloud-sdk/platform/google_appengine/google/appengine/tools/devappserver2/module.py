...
class ExternalModule(Module):
  ...
  def wsgi_server(self):
    if self._wsgi_server is None:
      raise ValueError('_wsgi_server is not initialized')
    return self._wsgi_server
  ...
  def suspend(self):
    ...
    with self._instance_change_lock:
      ...
      self._suspended = True
      with self._condition:
        self.wsgi_server.set_error(404)
    return _THREAD_POOL.submit(
        self._suspend_instance, self._instance, self.wsgi_server.port
    )
  ...
  def resume(self):
    ...
    with self._instance_change_lock:
      ...
      self._instance = inst
      self.wsgi_server.set_app(
          functools.partial(self._handle_request, inst=inst)
      )
      self._port_registry.add(self.wsgi_server.port, self, inst)
    self._async_start_instance(self.wsgi_server, inst)
  ...
  def restart(self):
    ...
    logging.info('Waiting for instances to restart')
    self._start_instance(self.wsgi_server, inst)
    logging.info('Instances restarted')
